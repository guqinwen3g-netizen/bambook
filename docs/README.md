...
| 工件 | 存放位置 | 用途 |
|---|---|---|
| 主链验收矩阵（28 节点） | 本文档下方「W-0 工件一」 | S1 双主链走查的逐节点判定表（死胡同条件/操作步骤/验收断言） |
| W-A 切片卡（5 张） | 本文档下方「W-0 工件二」 | 收官并行分工：4 张域卡并行 + 1 张四批次专项卡串行 |
| 跨域契约表（22 行） | 本文档下方「W-0 工件三」 | W-B 对账引擎的字段比对真源，含 4 个必须先拍板的断层 |

**W-B 开工前必须先拍板的 4 个断层**（契约表挖出）：
