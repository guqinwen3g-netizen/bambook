...
const petWindowPath = new URL('../AgentPetWindow.tsx', import.meta.url);
...
describe('BambookPandaAgent rig contract', () => {
  ...
  it('is used as the agent pet mascot instead of the old static brand mark', () => {
    const source = readFileSync(petWindowPath, 'utf8');

    expect(source).toContain("from './mascot/BambookPandaAgent'");
    expect(source).toContain('<BambookPandaAgent');
