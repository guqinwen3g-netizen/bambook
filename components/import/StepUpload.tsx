        ...
        >
          ...
          >
            ...
            <button
              ...
              onClick={clearAll}
              disabled={isParsing}
              className="text-[var(--text-tertiary)] hover:text-[var(--text-tertiary)] disabled:opacity-30"
            >
              全部清空
            </button>
          </div>
            ...
            <AnimatePresence initial={false}>
              {files.map((f) => (
                <motion.li
                  ...
                  } border-t border-[var(--border-c-subtle)]`}
                >
                  <FileText
                    size={18}
...
const StatusBadge: React.FC<{ status: FileStatus }> = ({ status }) => {
  switch (status) {
    ...
    default:
      return <span className="text-[var(--text-tertiary)] text-xs">待处理</span>;
  }
};
