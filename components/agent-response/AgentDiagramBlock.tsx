  ...
  return (
    <div className={`${OS_MATERIAL.insetSurface} rounded-inset border px-4 py-3 ${borderClass}`}>
      <div className="flex items-center justify-between gap-3">
        <div className={`text-xs uppercase tracking-widest ${labelTextClass}`}>{block.title ?? '图示'}</div>
      ...
      <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">
        {block.nodes.map(node => (
          <div key={node.id} className={`min-w-0 rounded-compact border px-3 py-2 ${borderClass}`}>
            ...
            {node.subtitle && <div className={`mt-0.5 truncate text-xs ${quietTextClass}`}>{node.subtitle}</div>}
          </div>
        ))}
      </div>
        ...
        <div className={`mt-3 rounded-compact border px-3 py-2 ${borderClass}`}>
          ...
          <div className="mt-2 flex flex-col gap-1.5">
            {block.edges.slice(0, 6).map((edge, index) => (
              <div key={`${edge.from}-${edge.to}-${index}`} className={`flex min-w-0 items-center gap-2 text-xs ${quietTextClass}`}>
            ...
            {block.edges.length > 6 && <div className={`text-xs ${quietTextClass}`}>还有 {block.edges.length - 6} 条关系未展开。</div>}
          </div>
        </div>
      )}
    </div>
