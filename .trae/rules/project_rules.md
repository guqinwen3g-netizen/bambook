...
- 当前波次：**W-C 权限收口近关闭**（代码级走查三套件全绿：permissionViewMatrix 242 例/permissionDenyPath 23 例/permissionAgentToolMatrix 21 例；30 页权限销号 30/30（page-consistency-ledger §7）；已交付：批一真源 `711462d`/`031892a`（GAP-R11 七角色收编+legacy 最小权限化+存量迁移已执行）、批二门禁 `9b66192`/`6329303`（4 视图补门+7 域 71 端点 scope 门）、批三 `81110f3`/`915e2ba`/`8b3adcf`（矩阵文档对齐+视图门真源派生根除双源漂移+族 B/C 缺口清零+Agent 工具层矩阵派生+HR 死胡同闭环）。**剩余**：S3 七角色实机走查（demo 账号 7 角色已 seed，需 dev server 实机验证）+ Mac Mini 部署执行 retire-finance-manager-role.ts）
- **功能修复任务包 40 项已全量交付（2026-08-27，最高优先级插队完成）**：批次 A 追溯链+硬 bug 4 项 `e499a6e`（A1 采购库存链入图+purchaseToStock 场景/A2 拖拽持久化/A3 盘点防护/A4 转单删除拦截 CONVERTED_TO_ORDER）→ 批次 B 门禁绕过 4 项 `648f8e4`（B1 阶段流转防绕过/B2 询价黑名单/B3 锁定量拦出库/B4 MOQ 建单提醒）→ 批次 C/D/E 31 项五车道并行 `1629120`（关系智库 CRM 7/供应商采购 8/数字档案 4/开发报价 6/订单生产 QC 6；E2 手机端冻结豁免）。下一步：S3 七角色实机走查
...
- 测试套件：后端 `cd server && npx vitest run`（4766 全绿，338 文件，含安全/幂等/并发/KB CRUD/MOQ 双触发/交期锁死/marketing/shared 补底/信用CAS/DR-013 门禁/QA-SEC 权限收口/DR-016 合票分配/出运批次门禁+结算自动触发挂接/催款分级穿透/专属面料四入口+行级预检+询价状态机/L8 回写幂等/退换货状态机/KPI 校验/信用门禁/确认路径统一/四单对账回归/直付旁路门禁/信用例外闭环/多币种对账回归/断层④双状态机挂钩/W-C 权限三套件（视图矩阵 242+路由反向 23+Agent 工具 21）/40 项任务包新增 150+ 例（追溯链/黑名单/锁定量/行级收货/放行口径统一等）/批次二新增约 180 例（运单状态机契约/退税审核身份/四类打印模板/组合留档/币种合并/佣金区间/薪酬门禁/离职停号/邮件正文附件入库/IMAP 会话仓/邮件自动同步/评级重估/合规轮询/汇率离线兜底/禁运国配置等）；注意 riskRoute 与 systemConfigRoute 存在偶发 flake，复跑即绿）；前端 `npm test`（3058 通过 + 13 项隔离，131 文件）
