...
- 测试套件：后端 `cd server && npx vitest run`（3887/3887 全通过，269 文件，含安全/幂等/并发/KB CRUD/MOQ 双触发/交期锁死/marketing/shared 补底/信用CAS/DR-013 门禁/QA-SEC 权限收口/DR-016 合票分配回归测试）；前端 `npm test`（2447 通过 + 6 项视觉基线 quarantine 隔离，103 文件）
